...
class DiagnosticsTests(unittest.TestCase):
    def test_failure_evidence_redacts_and_bounds(self):
        ...
        self.assertEqual(m.failure_evidence([])['process_counts'], {})
        self.assertEqual(m.failure_evidence({'receipt_exists': True, 'process_counts': []})['receipt_exists'], True)
        ...
        self.assertLessEqual(len(m.failure_evidence({'log_tail': ('error '*60+'\n')*100})['sanitized_log_tail']), 8000)
